...
function (repfile = "ss2.rep", cl = NULL, tb = NULL, target.line = NULL) 
{
    ...
    if (is.null(target.line)) {
        ...
        name.label <- find.and.read.table2(read.char, skipline = 0, 
            gyou = 1, table.property = cl, tb = tb, outfile = repfile, 
            h = FALSE, is.ss2 = TRUE, colClasses = "character")
        res <- find.and.read.table2(read.char, skipline = 1, 
            gyou = NULL, table.property = cl, tb = tb, outfile = repfile, 
            h = FALSE, is.ss2 = TRUE, comment.char = "")
    }
    else {
        name.label <- find.and.read.table(read.char, skipline = 0, 
            startpoint = target.line - 10, gyou = 1, table.property = cl, 
            outfile = repfile, h = FALSE, is.ss2 = TRUE, colClasses = "character")
        res <- find.and.read.table(read.char, skipline = 1, startpoint = name.label[[2]] - 
            10, gyou = NULL, table.property = cl, comment.char = "", 
